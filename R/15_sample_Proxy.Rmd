...
for(mciter in 1:NMC){
  ...
  for(i in 1:N){
    ...
    Xbeta[[i]] <- Xbeta_0[[i]] + Zt[[i]] %*% mc$alph[mciter,,i]
...
#   
  ...
  c[[1]] <- t(Zt[[1]]) %*% drop0(A11[[1]] %*% (y[[1]]-Xbeta[[1]])) -
    rho * (Q %*% Wbeta[[1]]) + 
    P1inv %*% matrix(mu_1, Size,1)
  for(t in 2:(N-1)){
    c[[t]] <- t(Zt[[t]]) %*% drop0(A11[[t]] %*% (y[[t]]-Xbeta[[t]])) -
      rho * (Q %*% Wbeta[[t]]) +  (Q %*% Wbeta[[t-1]])
  }
  c[[N]] <- t(Zt[[N]]) %*% drop0(A11[[N]] %*%y[[N]]-Xbeta[[N]]) +
    Q %*% Wbeta[[t-1]]
...
#   for(t in 1:N){OSO[[t]] <- crossprod(LO[[t]])}
  ...
  for(spec in 1:19){ # for each tree species (the instruments are spec==1)
    ...
    yy <- unlist(mapply(function(x,y) x[y==spec],y, species_ids))
    xx <- unlist(mapply(function(x,y,z) as.vector((Z[y,] %*% x))[z==spec], split(alpha, rep(1:ncol(alpha), each=nrow(alpha))), site_ids, species_ids))
...
library(scales)
spatial.coords %>% mutate(temp=as.vector(A.inst %*% mc$alpha[100,,150])) %>% 
  ggplot() + geom_point(aes(x=lon, y=lat, color=temp), size=5) + 
  scale_color_gradient2(low=muted("blue"), high=muted("red"), midpoint=0) + 
  coord_map(projection='mollweide')

anom.df %>% filter(year==years[150]) %>% ggplot()+geom_point(aes(x=lon, y=lat, color=sas), size=5)+ scale_color_gradient2(low=muted("blue"), high=muted("red"), midpoint=0) +   coord_map(projection='mollweide')
...
ggplot(data=T)+geom_point(aes(x=-lon, y=lat, color=beta1)) + scale_color_gradient2(low=muted("blue"), high=muted("red"), midpoint=0) +   coord_map(projection='mollweide')
```
