...
#  tau.sq.lat.year.1 <- 1e-8
 ...
 rm(c)
  ...
  alpha[[N]] <- thompson$m[[N]] + backsolve(thompson$Lambda[[N]], epsilon, upper.tri=FALSE, transpose=TRUE)
  
  for(t in seq(N-1, 1, -1)){
    epsilon <- rnorm(n=nrow(Q))
    epsilon <- epsilon - thompson$LambdaOmega[[t]] %*% alpha[[t+1]]
    alpha[[t]] <- thompson$m[[t]] + backsolve(thompson$Lambda[[t]], epsilon, 
...
#     }
  ...
  for(spec in 1:length(species_levels)){
    ...
    yy <- iter.df$prox[rows.by.species[[spec]]]
...
#       beta.sim[c(1:8, 65:72)] <- as.vector(Z %*% (sim_mvn_can(t(XXZ) %*% yy, t(XXZ) %*% XXZ + ZKZ) ))
      # beta.sim <- as.vector(Z %*% (sim_mvn_can(t(XXZ) %*% yy, t(XXZ) %*% XXZ + ZKZ) ))
